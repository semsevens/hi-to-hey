...
var chat = (function() {
    ...
    function updateChatHeight() {
        $messagesContainer.css({
            height: 400 - $(".chat-input-bar").height()
        });
    }
    ...
    function addMessage(data, self) {
        var $messageContainer = $("<li/>")
            .addClass('chat-message new ' + (self ? 'chat-message-self' : 'chat-message-friend'))
            .appendTo($messagesList);
        var $messageBubble = $("<div/>")
            .addClass('chat-message-bubble')
            .appendTo($messageContainer);
        // var time = (new Date(data.created)).toLocaleTimeString(navigator.language, {
        // hour12: false
        // });
        var messageInfo = '';
        if (!self)
            messageInfo = '<div class="my-message-info">' + data.username + '：</div>';
        $messageBubble.html(messageInfo + data.content);
    ...
    function sendMessage(data) {
        ...
        var messageElements = addMessage(data, true),
    ...
    function receiveMessage(data) {
        var messageElements = addMessage(data, false),
            $messageContainer = messageElements.$container,
            $messageBubble = messageElements.$bubble;
